...
export class DarwinKit implements DarwinKitClient {
    ...
    return new Promise<ReadyNotification>((resolve, reject) => {
      ...
      this.transport.start({
        ...
        onExit: (code: number | null) => {
          ...
          if (!readyReceived) {
            reject(
              new Error(`darwinkit exited with code ${code} before ready`),
            );
            return;
          }
  ...
  private handleLine(
    ...
    if (!readyReceived && msg.method === "ready") {
      ...
      this.log(
        "info",
        `Connected (v${params.version}, ${params.capabilities.length} methods)`,
      );
  ...
  private async attemptReconnect(): Promise<void> {
    ...
    while (this.reconnectAttempt < this.reconnectConfig.maxRetries) {
      ...
      this.log(
        "warn",
        `Reconnecting (attempt ${this.reconnectAttempt}/${this.reconnectConfig.maxRetries})...`,
      );
      ...
      try {
        ...
        for (const [oldId, req] of held) {
          ...
          this.call(
            req.method as MethodName,
            req.params as MethodMap[MethodName]["params"],
          )
            .then(req.resolve)
            .catch(req.reject);
        }
